...
{
	for(size_t i = 0; i < N; i+=cache_blocking_size){
		for(size_t k = 0; k < N; k+=cache_blocking_size){
			for(size_t j = 0; j < N; j+=cache_blocking_size){
				for (size_t i1 = i; i1 < i+cache_blocking_size; i1++) {
					for (size_t k1 = k; k1 < k+cache_blocking_size; k1++) {
						for (size_t j1 = j; j1 < j+cache_blocking_size; j1++) {
							o_C[i1*N + j1] += i_A[i1*N + k1]*i_B[k1*N + j1];
...
{
	int cache_blocking_size = CACHE_CONST_BLOCKING_SIZE;
	for(size_t i = 0; i < N; i+=cache_blocking_size){
		for(size_t k = 0; k < N; k+=cache_blocking_size){
			for(size_t j = 0; j < N; j+=cache_blocking_size){
				for (size_t i1 = i; i1 < i+cache_blocking_size; i1++) {
...
void matrix_setup_A(std::size_t N, double *M)
{
	...
	for (std::size_t i = 0; i < N; i++)
	{
		for (std::size_t j = 0; j < N; j++)
		{
			M[i*N + j] = std::cos((i+0.5)*(j+0.5)*M_PI/N);
		}
	}
}
...
void matrix_setup_B(std::size_t N, double *M)
{
	...
	for (std::size_t i = 0; i < N; i++)
	{
		for (std::size_t j = 0; j < N; j++)
		{
			M[i*N + j] = std::cos((i+0.5)*(j+0.5)*M_PI/N)*2/N;
		}
	}
}
